	...
	public static class Modifiers {
		...
		public class Blotches : GenAction {
			...
			public override bool Apply(Point origin, int x, int y, params object[] args) {
				...
				if (_random.NextDouble() < _chance) {
					...
					int num  = _random.Next(1 - _minX, 1);
					int num2 = _random.Next(0, _maxX);
					int num3 = _random.Next(1 - _minY, 1);
					int num4 = _random.Next(0, _maxY);
		...
		public class IsTouchingAir : GenAction {
			...
			public override bool Apply(Point origin, int x, int y, params object[] args) {
				...
				for (int i = 0; i < num; i += 2)
					if (!_tiles[x + DIRECTIONS[i], y + DIRECTIONS[i + 1]].active())
						return UnitApply(origin, x, y, args);
		...
		public class IsSolid : GenAction {
			...
			public override bool Apply(Point origin, int x, int y, params object[] args) {
				...
				if (_tiles[x, y].active() && WorldGen.SolidOrSlopedTile(x, y))
					return UnitApply(origin, x, y, args);
		...
		public class Dither : GenAction {
			...
			public override bool Apply(Point origin, int x, int y, params object[] args) {
				...
				if (_random.NextDouble() >= _failureChance)
					return UnitApply(origin, x, y, args);
