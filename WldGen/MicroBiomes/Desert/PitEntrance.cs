		...
		private static void PlaceAt(DesertDescription description, Point position, int holeRadius) {
			...
			for (int i = -holeRadius - 3; i < holeRadius + 3; i++) {
				...
				for (int j = description.Surface[i + position.X]; j <= description.Hive.Top + 10; j++) {
					...
					float value = (j - description.Surface[i + position.X]) / (description.Hive.Top - description.Desert.Top);
					value = MathHelper.Clamp(value, 0f, 1f);
					int num = (int)(GetHoleRadiusScaleAt(value) * holeRadius);
					
					if (Math.Abs(i) < num) {
						DrawInterface.AddChangedTile(i + position.X, j);
					...
					float num2 = Math.Abs(i / holeRadius);
					num2 *= num2;

					if (Math.Abs(i) < num + 3 && (j - position.Y) > 15f - 3f * num2) {

						DrawInterface.AddChangedTile(i + position.X, j);
						Main.tile[i + position.X, j].wall = 187;
						Neo.SetWall(i + position.X, j, 187, ref DrawInterface.Undo);
			...
			for (int k = -holeRadius; k < holeRadius; k++) {
				...
				for (int l = 0; l < num3; l++) {
					DrawInterface.AddChangedTile(k + position.X, l + description.Surface[k + position.X]);
