				...
				foreach (List<Point> item5 in list) {
					...
					if (item5.Count > 0) {
						...
						foreach (Point item6 in item5)
							cluster.Add(new Block(item6.X + (WorldGen.genRand.NextFloat() - 0.5f) * 0.5f, item6.Y + (WorldGen.genRand.NextFloat() - 0.5f) * 0.5f));
			...
			for (int i = hive.Left; i < hive.Right; i++) {
				...
				for (int j = hive.Top; j < hive.Bottom; j++) {
					...
					if (postPlacementEffect.HasFlag(PostPlacementEffect.Smooth))
						WldGen.SmoothSlope(i, j, ref DrawInterface.Undo, applyToNeighbors: false);
			...
			for (int i = area.Left; i < area.Right; i++) {
				...
				for (int j = area.Top; j < area.Bottom; j++) {
					...
					if (num7 > 3.5f) {
						...
						if (j < WorldGen.lavaLine) { // Was Main.worldSurface
							tile.liquid = 0;
						}
						else {
							//tile.lava(lava: true);
			...
			for (int k = -20; k < description.Hive.Width + 20; k++) {
				...
				for (int l = -20; l < description.Hive.Height + 20; l++) {
					...
					/*if (flag && WorldGen.genRand.Next(5) == 0) {
						...
						PlaceTile(num3, num4 - 1, 484, ref DrawInterface.Undo, mute: true, forced: true);
					}
					else */if ((flag ^ flag2) && WorldGen.genRand.Next(5) == 0) {
						PlaceTile(num3, num4 + ((!flag) ? 1 : (-1)), 165, ref DrawInterface.Undo, mute: true, forced: true);
					}
					else if (flag && WorldGen.genRand.Next(5) == 0) {
						PlaceTile(num3, num4 - 1, 187, ref DrawInterface.Undo, mute: true, forced: true, -1, 29 + WorldGen.genRand.Next(6));
					}
