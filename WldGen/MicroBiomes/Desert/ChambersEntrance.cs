...
namespace NeoDraw.WldGen.MicroBiomes {
	...
	public static class ChambersEntrance {
		...
		private static void PlaceAt(DesertDescription description, Point position) {
			...
			Gen(
				origin,
				new Shapes.Circle(24, 12),
				Actions.Chain(
					new Modifiers.Blotches(),
					new Actions.SetTile(53).Output(shapeData)
				)
			);
			...
			for (int i = 0; i < num3; i++) {
				...
				int num4 = ((i + 1) / num3 * num) + genRand.Next(-8, 9);
				int num5 = num2 * genRand.Next(20, 41);
				int num6 = genRand.Next(18, 29);
				
				Gen(
					position,
					new Shapes.Circle(num6 / 2, 3),
					Actions.Chain(
			...
			for (int j = 1; j < list.Count; j++) {
				...
				for (float num8 = 0f; num8 <= 1f; num8 += 0.02f) {
					...
					Gen(
						Vector2.Lerp(value3, value4, num8).ToPoint(),
						pair
					);
			...
			Gen(
				origin,
				new Shapes.Rectangle2(new Rectangle(-29, -12, 58, 12)),
				Actions.Chain(
